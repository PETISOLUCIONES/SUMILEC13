...
class ImportXML(models.TransientModel):
    ...
    def CreatedSupplier(self, xml_arc, NSMAP):
        ...
        resPartner = self.env['res.partner'].create({
            ...
            'city_id': self.env['res.country.state.city'].search([('code', '=', city_id)]).id,
            'city': self.env['res.country.state.city'].search([('code', '=', city_id)]).name,
            'state_id': self.env['res.country.state'].search([('dian_state_code', '=', state_id)]).id,
            'street': street,
            'l10n_co_document_type': type_document,
            'vat_num': vat_num,
            'vat_vd': vat_vd,
            'phone': tel_supplier,
