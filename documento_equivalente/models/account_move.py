...
class AccountMove(models.Model):
    ...
    def GetInvoiceType(self, datos):
        ...
        for dato in datos:
            if res == '':
                if dato.type == 'in_invoice' and dato.documento_equivalente:
                    return '05'
                elif dato.type == 'in_refund' and dato.documento_equivalente:
                    return '95'
            return res
        ...
        if invoicetype in ['05', '95']:
            if move.invoice_origin:
                ...
                if not invoice_origin:
                    raise UserError("La factura debe tener una orden de compra relacionada")
            else:
                raise UserError("La factura debe tener una orden de compra relacionada")
            datos['datos'][0]['DocPeriod'] = move.doc_period.code if move.doc_period.code else ' '
            datos['datos'][0]['PurchaseDate'] = invoice_origin.date_approve.strftime('%d/%m/%Y %H:%M:%S')
        return datos
    ...
    def create_dict_company_dian(self, move):
        ...
        if invoicetype in ['05', '95']:
            ...
            return {'company': CustNum,
                    ...
                    'OrderNum': invoice_origin.name,
                    'DateOrder': invoice_origin.date_approve.strftime('%d/%m/%Y %H:%M:%S'),
