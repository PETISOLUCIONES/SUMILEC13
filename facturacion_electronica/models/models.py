...
class AccountMove(models.Model):
    ...
    def _get_report_base_filename(self):
        ...
        if any(move.url_pdf and move.invoice_status_dian == "Exitoso" for move in self):
            raise UserError(
                _("Esta factura se encuentra aceptada por la DIAN. Por favor utilice la representación gráfica."))
    ...
    def create_dict_company_dian(self, move):
        ...
        nit_company = move.GetNitCompany(move.company_id.vat, move.company_id.country_id.code)
        ...
        if invoice_type == '91':
            ...
            return {'InvoiceRef': move.debit_origin_id.name,
                    ...
                    'ResolutionDateInvoice': move.journal_id.sequence_id.resolution_id.resolution_resolution_date.strftime(
                        '%Y-%m-%d'),
                    'ResolutionDateFrom': move.journal_id.sequence_id.resolution_id.resolution_date_from.strftime(
                        '%Y-%m-%d'),
                    'ResolutionDateTo': move.journal_id.sequence_id.resolution_id.resolution_date_to.strftime(
                        '%Y-%m-%d'),
    ...
    def create_dict_invoicehead_dian(self, move):
        ...
        datos = dict(Company=nit_company,
                     ...
                     DueDate=move.invoice_date_due.strftime('%d/%m/%Y') + " " + now.strftime('%H:%M:%S'),
                     ...
                     InvoiceComment4=resolucion['CMReasonDesc_c'] if invoicetype == '91' else resolucion[
                         'DMReasonDesc_c'] if invoicetype == '92' else "",
        ...
        for move in self:
            ...
            if responsews.status_code == 200:
                ...
                if respuestaws == 'PROCESADO_CORRECTAMENTE':
                    ...
                    zf = zipfile.ZipFile(directoriozip + attach[0:len(attach) - 4].replace('ad', 'z') + '.zip',
                                         mode='w')
                    try:
    ...
    def GetInvoiceType(self, datos):
        ...
        for dato in datos:
            ...
            elif dato.type == 'out_invoice' and dato.debit_note == False:
...
class ResPartner(models.Model):
    ...
    partner_currency_id = fields.Many2one('res.currency', string="Currency",
