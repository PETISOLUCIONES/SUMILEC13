...
class Invoice(models.Model):
    ...
    ], string='Estado de acuse', readonly=True, default='030')
    ...
    dias_recibo = fields.Integer(string='dias recibo', compute='_get_dias_recibo', default=0)
    ...
    def EnviarAcuse(self):
        ...
        for move in self:
            ...
            directorio = "Facturacion/Acuses/" + move.GetNitCompany(move.company_id.vat, move.company_id.country_id.code) + "/"

            if move.env.company.ruta_plantilla_acuse:
                ruta_xml = move.env.company.ruta_plantilla_acuse + "/Acuses/" + move.GetNitCompany(move.company_id.vat, move.company_id.country_id.code) + "/"
            else:
                ruta_xml = directorio
    ...
    def EnviarAceptacionTacita(self):
        ...
        for move in self:
            ...
            directorio = "Facturacion/Acuses/" + self.GetNitCompany(self.company_id.vat, self.company_id.country_id.code) + "/"

            if self.env.company.ruta_plantilla_acuse:
                ruta_xml = self.env.company.ruta_plantilla_acuse + "/Acuses/" + self.GetNitCompany(
                    self.company_id.vat, self.company_id.country_id.code) + "/"
            else:
                ruta_xml = directorio
    ...
    def EncabezadoAcuse(self, move):
        ...
        if move.state_acuse == '030':  # acuse
            ...
            Note = "Manifiesto bajo la gravedad de juramento que transcurridos 3 días hábiles contados desde " \
                   ...
                   "la aceptación o rechazo de la referida factura, ni reclamó en contra de su contenido".format(
                acuse_bienes.name, acuse_bienes.cude, move.partner_id.name, move.partner_id.vat)
        ...
        datos = dict(Company=move.GetNitCompany(move.company_id.vat, move.company_id.country_id.code),
                     AcsType=AcsType,
                     AcsNum=AcsNum,
    ...
    def DatosCompany(self, move):
        ...
        datoscompany = dict(company=move.GetNitCompany(move.company_id.vat, move.company_id.country_id.code),
    ...
    def DatosSupplier(self, move):
        # Supplier
        Company = move.GetNitCompany(move.company_id.vat, move.company_id.country_id.code)
        CustID = move.TypeDocumentCust(move.partner_id.l10n_co_document_type)
        CustNum = move.GetNitCompany(move.partner_id.vat, move.partner_id.country_id.code)
    ...
    def DatosPerson(self, move):
        ...
        PersonNum = move.GetNitCompany(move.company_id.vat, move.company_id.country_id.code)
